...
void listar_produtos(Produto lista[], int total) {
    ...
    printf("ID: %s | Codigo Barras: %s | Categoria: %s | Nome: %s | Preco: R$ %.2f | Qtd: %d", lista[i].id, lista[i].codigo_barrras, lista.[i].categoria, lista[i].nome, lista[i].preco, lista[i].quantidade);
    }
}
...
int main(void) {
    ...
    estoque[1].categoira = "Escritorio";
